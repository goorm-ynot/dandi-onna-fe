...
export async function DELETE (request: NextRequest) {
    try {
        ...
        // 백엔드 API 호출
        const response = await serverApiClient.delete(`/owner/no-show-post-schedules/${scheduleId}`);
        
        return NextResponse.json(response, { status: 200 });
    } catch (error: any) {
        console.error('❌ Error cancelling no-show post schedule:', error);
        // 인터셉터에서 토큰 재발급을 처리하므로 여기서는 에러만 반환   
        if (error?.code === 403) {
            return NextResponse.json(
                {
        ...
        return NextResponse.json({ error: error.message || 'Failed to cancel no-show post schedule' }, { status: error?.code || 500 });
    }
}